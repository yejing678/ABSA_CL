...
class Instructor:
    ...
    def _train(self, CE, SCL, optimizer, train_data_loader, val_data_loader):
        print('staring train...')
        ...
        path = None
        Path(
            f"plots/SCL_train/{self.opt.model_name}/{self.opt.dataset}/{self.opt.lamda}/{self.opt.seed}") \
            .mkdir(parents=True, exist_ok=True)
        writer = open(
            f"plots/SCL_train/{self.opt.model_name}/{self.opt.dataset}/{self.opt.lamda}/{self.opt.seed}/logs.csv",
            "w")
        ...
        for i_epoch in tqdm(range(self.opt.num_epoch)):
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                CE_loss = CE(outputs, aspect_sentiments)
                if not has_opposite_labels(aspect_sentiments):
                    SCL_loss = 0
                else:
                    normed_cls_hidden = F.normalize(outputs_feature, dim=-1)
                    SCL_loss = SCL(normed_cls_hidden.unsqueeze(1), labels=aspect_sentiments)
                ...
                if step % self.opt.log_step == 0:
                    ...
                    if val_acc > max_val_acc:
                        ...
                        save_path = "state_dict/{0}/{1}/{2}/{3}/{4}/" \
                            .format(self.opt.model_name, self.opt.dataset,
                                    self.opt.local_context_focus, self.opt.lamda, self.opt.seed)
            ...
            train_picture.plot_jasons_lineplot(update_num_list, train_loss_list, 'updates', 'training loss',
                                               ...
                                               f"plots/SCL_train/{self.opt.model_name}/{self.opt.dataset}/{self.opt.lamda}/{self.opt.seed}/train_loss.png")
            train_picture.plot_jasons_lineplot(update_num_list, val_acc_list, 'updates', 'validation accuracy',
                                               f"{self.opt.model_name}  {self.opt.dataset}  max val acc={max(val_acc_list):.3f}",
                                               f"plots/SCL_train/{self.opt.model_name}/{self.opt.dataset}/{self.opt.lamda}/{self.opt.seed}/val_acc{max(val_acc_list):.3f}.png")
            train_picture.plot_jasons_lineplot(update_num_list, train_acc_list, 'updates', 'train accuracy',
                                               f"{self.opt.model_name}  {self.opt.dataset}  max val acc={max(val_acc_list):.3f}",
                                               f"plots/SCL_train/{self.opt.model_name}/{self.opt.dataset}/{self.opt.lamda}/{self.opt.seed}/train_acc.png")
    ...
    def run(self):
        ...
        self._reset_params()
        best_model_path = self._train(CE, SCL, optimizer, train_data_loader, val_data_loader)
...
def main():
    ...
    parser.add_argument('--lamda', default=0.5, type=float, help='between(0,1]')
    ...
    parser.add_argument('--seed', default=9, type=int, help='set seed for reproducibility')
    ...
    log_file = 'SCL-train:{}-{}-{}-{}-{}-{}-{}-{}.log'.format(opt.model_name, opt.dataset, opt.local_context_focus, opt.lr,
