...
class Instructor:
    ...
    def _reset_params(self):
        ...
        for child in self.model.children():
            if type(child) != BertModel:  # skip bert params
                ...
                self.model.load_state_dict(torch.load("/home/jye/ABSA/Curriculum Learning/state_dict/pre-train/bert_spc/yelp_restaurant/42/epoch15_step1250"), strict=False)


    def _train_CE_TL(self, criterion1, criterion2, optimizer, train_data_loader, val_data_loader):
        print('staring train...')
        max_val_acc, max_val_f1, max_val_epoch, global_step= 0, 0, 0, 0
        update_num_list, train_loss_list, train_acc_list, val_acc_list = [], [], [], []
        path = None
        Path(
            f"plots/triplet loss/{self.opt.model_name}/{self.opt.dataset}/{self.opt.lamda}/{self.opt.seed}/{self.opt.b}") \
            .mkdir(parents=True, exist_ok=True)
        writer = open(
            f"plots/triplet loss/{self.opt.model_name}/{self.opt.dataset}/{self.opt.lamda}/{self.opt.seed}/{self.opt.b}/logs.csv",
            "w")
        ...
        for i_epoch in tqdm(range(self.opt.num_epoch)):
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                # loss = (loss - self.opt.b).abs() + self.opt.b
                loss.backward()
                optimizer.step()
                ...
                if step % self.opt.log_step == 0:
                    ...
                    if val_acc > max_val_acc:
                        ...
                        save_path = "state_dict/triplet loss/{0}/{1}/{2}/{3}/{4}/" \
            ...
            train_picture.plot_jasons_lineplot(update_num_list, train_loss_list, 'updates', 'training loss',
                                               ...
                                               f"plots/triplet loss/{self.opt.model_name}/{self.opt.dataset}/{self.opt.lamda}/{self.opt.seed}/{self.opt.b}/train_loss.png")
            train_picture.plot_jasons_lineplot(update_num_list, val_acc_list, 'updates', 'validation accuracy',
                                               f"{self.opt.model_name}  {self.opt.dataset}  max val acc={max(val_acc_list):.3f}",
                                               f"plots/triplet loss/{self.opt.model_name}/{self.opt.dataset}/{self.opt.lamda}/{self.opt.seed}/{self.opt.b}/val_acc{max(val_acc_list):.3f}.png")
            train_picture.plot_jasons_lineplot(update_num_list, train_acc_list, 'updates', 'train accuracy',
                                               f"{self.opt.model_name}  {self.opt.dataset}  max val acc={max(val_acc_list):.3f}",
                                               f"plots/triplet loss/{self.opt.model_name}/{self.opt.dataset}/{self.opt.lamda}/{self.opt.seed}/{self.opt.b}/train_acc.png")
    ...
    def run(self):
        ...
        self._reset_params()
        best_model_path = self._train_CE_TL(criterion1, criterion2, optimizer, train_data_loader, val_data_loader)
        # best_model_path = self._train_CE(criterion1, optimizer, train_data_loader, val_data_loader)
...
def main():
    ...
    parser.add_argument('--num_epoch', default=8, type=int, help='try larger number for non-BERT models')
